...
from dataloader.dataloader import unshuffle_loader, unshuffle_loader_new
...
class SCCLvTrainer(nn.Module):
    ...
    def get_batch_token_new(self, text):
        token_feat = self.enc(text)
        return token_feat
    ...
    def train_step_virtual(self, input_ids, attention_mask):
        ...
        embd1 = embd1.cuda().float()
        embd2 = embd2.cuda().float()
    ...
    def train_step_explicit(self, input_ids, attention_mask):
        ...
        if self.args.objective == "SCCL":
            ...
            loss += cluster_loss
            losses["cluster_loss"] = cluster_loss.item()
            print("### {} {}".format(losses["loss"], losses["cluster_loss"]))
        loss.backward()
        self.optimizer.step()
        self.optimizer.zero_grad()
        return losses
    ...
    def train(self):
        ...
        for i in np.arange(self.args.max_iter+1):
            ...
            # 二个是virtual的
            # data = batch["text"].cuda().long()
            # input_ids = torch.cat([data.unsqueeze(1),data.unsqueeze(1)],dim=1)

            # 三个是aug的
            text1, text2, text3 = batch['text'].cuda().long(), batch['augmentation_1'].cuda().long(), batch['augmentation_2'].cuda().long()
            input_ids = torch.cat([text1.unsqueeze(1),text2.unsqueeze(1),text3.unsqueeze(1)],dim=1)
            
            attention_mask = torch.ones_like(input_ids).cuda().long()
            ...
            if (self.args.print_freq>0) and ((i%self.args.print_freq==0) or (i==self.args.max_iter)):
                ...
                torch.save({'model': self.model.state_dict()}, self.args.resPath + 'model_'+ str(i)  +'.pth')
                self.model.train()
    ...
    def evaluate_embedding(self, step):
        ...
        dataloader = unshuffle_loader_new(self.args)
        ...
        for i, batch in enumerate(dataloader):
            with torch.no_grad():
                ...
                feat = self.model(text.cuda().long(), torch.ones_like(text).cuda().long(), task_type="evaluate")
...
class BertvTrainer(nn.Module):
    def __init__(self, model, tokenizer, optimizer, train_loader, args):
        super(BertvTrainer, self).__init__()
        self.model = model
        self.tokenizer = tokenizer
        self.optimizer = optimizer
        self.train_loader = train_loader
        ...
        # self.enc = nn.Linear(40,40)
        self.gstep = 0
        print(f"*****Intialize Bertv, temp:{self.args.temperature}, eta:{self.args.eta}\n")
    ...
    def train_step(self, input_ids, attention_mask):
        ...
        embd1, embed2, embed3 = self.model(input_ids, attention_mask, task_type="explicit")
        loss = 0
    ...
    def train(self):
        ...
        for i in np.arange(self.args.max_iter+1):
            ...
            # 三个是aug的
            text1 = batch['text'].cuda().long()
            input_ids = torch.cat([text1.unsqueeze(1),text1.unsqueeze(1),text1.unsqueeze(1)],dim=1)
