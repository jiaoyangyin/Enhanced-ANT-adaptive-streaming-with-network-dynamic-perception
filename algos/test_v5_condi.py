...
from .beta_vae_v6_condi import BetaVAE
from .AC_net_v6 import Actor
from api.Transformers import SCCLBert, get_uniform, GLOBAL_OPEN, update_state, load_model
from api.optimizer import get_bert
# from .train_ppo_v6 import get_uniform
import time
...
# @jit(forceobj=True)
def evaluation(actor_net, vae_net, sccl, log_path_ini, test_env, \
    ...
    for video_count in tqdm(range(len(all_file_name))):
        while True:
            ...
            start = time.time()
            if GLOBAL_OPEN:
                tmp, input_seq = update_state(seq_s, seq_t, state, time_stamp, sccl)
                ob[0, :] = input_seq
                # print(tmp)
                # print(input_seq)
                if last_time_stamp == 0:
                    ob[1, 0:5] = torch.Tensor([0, 0, 0, 0, 0])
                if time_stamp - last_time_stamp >= 20 * 1e3:
                    last_time_stamp = time_stamp
                    ob[1, 0:5] = tmp.detach().cpu().numpy()
                else:
                    ob[1, 0:5] = last
            else:
            ...
            # print("sccl infer time:", end - start)
...
def valid(args, env, actor_net, vae_net, sccl, epoch, log_file, add_str, summary_dir):
    ...
    evaluation(actor_net, vae_net, sccl, log_path_ini, env, \
                    s_info, s_len, c_len, total_chunk_num, \
                        bitrate_versions, rebuffer_penalty, smooth_penalty, a_dim, args)
...
def test(args, test_model, env, log_file, add_str, summary_dir):
    ...
    print(vae_dim)
    model_vae = BetaVAE(in_channels=vae_dim, hist_dim=c_len, latent_dim=latent_dim).type(dtype)
    model_actor.eval()
    model_vae.eval()
    ...
    evaluation(model_actor, model_vae, sccl, log_file, env, \
        s_info, s_len, c_len, total_chunk_num, \
                bitrate_versions, rebuffer_penalty, smooth_penalty, a_dim, args)
