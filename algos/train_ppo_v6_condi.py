...
from .test_v5_condi import valid
    ...
    with open(log_file_name + '_record', 'w') as log_file, \
        ...
        while True:
            ...
            for _ in range(ppo_ups):
                ...
                total_loss_.append(loss_actor.detach().cpu().numpy())
            ...
            writer.add_scalar("Avg_Actor_loss", np.mean(total_loss_), epoch)
            print(f"=== === {epoch} {np.mean(vae_kld_loss_)} {np.mean(policy_loss_)} {np.mean(value_loss_)} {np.mean(entropy_loss_)} {np.mean(policy_mi_loss_)} {np.mean(session_return)} {np.mean(total_loss_)}\n")
            writer.flush()

            if epoch % int(args.valid_i /10) == 0 and epoch > 0:
                logging.info("Model saved in file")
                valid(args, valid_env, model_actor, vae_net, sccl, epoch, \
